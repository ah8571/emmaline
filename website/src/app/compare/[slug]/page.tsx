...
function getBaseSlug(slug: string) {
  return slug.endsWith('-vs-oov') ? slug.slice(0, -'-vs-oov'.length) : slug;
}
...
export function generateMetadata({ params }: ComparisonPageProps): Metadata {
  ...
  return {
    title: `${competitor.name} vs oov | AI Phone Assistant Comparison`,
    description: `Compare ${competitor.name} and oov across voice-first experience, assistant identity, and everyday AI phone assistant use cases.`,
    alternates: {
...
export default function ComparisonPage({ params }: ComparisonPageProps) {
  ...
  const toc = [
    ...
    { id: 'oov-difference', label: 'Where oov is different' },
  ...
  return (
    <SeoArticleLayout
      ...
      title={`${competitor.name} vs oov`}
      intro={`This comparison looks at ${competitor.name} through oov's intended lane: a practical AI phone assistant for real conversations, everyday help, and note-friendly voice workflows instead of a generic chatbot experience.`}
      toc={toc}
    >
      ...
      {isCallAnnie ? (
        <section id="current-status" className="space-y-4 rounded-3xl border border-white/10 bg-white/[0.03] p-6 md:p-8">
          ...
          <p className="leading-8 text-white/60">
            That is enough to say the old app is no longer live in the normal consumer-download sense. It is not enough to say why the product was discontinued, so this comparison should stay focused on the clearer question: if someone liked the original call-an-AI idea, why might oov be a better current alternative?
          </p>
        </section>
      ) : null}
      ...
      <section id="oov-difference" className="space-y-4">
        <h2 className="text-2xl font-semibold md:text-3xl">Where oov is different</h2>
        <p className="leading-8 text-white/70">{competitor.oovAngle}</p>
        <p className="leading-8 text-white/60">
          {isCallAnnie
            ? 'The strongest alternative angle is not that oov recreates a discontinued app one-for-one. It is that oov can keep the low-friction call-first mental model while adding a note-friendly workflow: call in, think out loud, brainstorm in real time, and keep the useful parts through transcripts, call detail, and notes.'
            : 'The part the generic voice products usually skip is the note-friendly workflow. oov can be judged on a tighter loop: call in, think out loud, brainstorm in real time, and keep the useful parts through transcripts, call detail, and notes. That pushes it closer to a practical phone assistant than a broad voice demo or companion chat.'}
        </p>
      </section>
      ...
      <section className="grid gap-6 md:grid-cols-2">
        ...
        <div className="rounded-3xl border border-white/10 p-6 md:p-8">
          <h2 className="text-2xl font-semibold">Why users might still want oov</h2>
          <ul className="mt-4 space-y-3 text-white/70">
      ...
      <section id="best-fit" className="space-y-4">
        ...
        <p className="leading-8 text-white/70">
          {competitor.name} is a stronger fit for {competitor.audience.toLowerCase()} oov is a stronger fit for users who want a more focused AI phone assistant feel, direct phone-style interaction, and note-taking around conversations instead of a broad chat surface or companion product.
        </p>
      </section>
      ...
      <section id="related-reads" className="rounded-3xl border border-white/10 bg-white/[0.03] p-6 md:p-8">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {relatedComparisons.map((entry) => (
            <Link
              key={entry.slug}
              href={`/compare/${entry.slug}-vs-oov`}
            ...
            >
              ...
              <p className="mt-2 text-lg font-semibold text-white">{entry.name} vs oov</p>
